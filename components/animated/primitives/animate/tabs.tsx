...
import {
  ...
  type HighlightProps,
} from "@/components/animated/primitives/effects/highlight";
import { getStrictContext } from "@/lib/get-strict-context";
import {
  Slot,
  type WithAsChild,
} from "@/components/animated/primitives/animate/slot";
...
function TabsContents({
  ...
  }, [activeIndex, childrenArray.length, measure, animateHeight]);
  ...
  }, [activeIndex, height, measure, animateHeight]);

  const mergedStyle = animateHeight
    ? { overflow: "hidden" as const, ...styleProp }
    : { overflow: "hidden" as const, ...styleProp };

  const mergedAnimate = animateHeight
    ? { height, ...(typeof animateProp === "object" ? animateProp : {}) }
    : (animateProp ?? {});
  ...
  return (
    <motion.div
      ...
      transition={animateHeight ? transition : undefined}
      {...props}
    >
      <motion.div
        className={`flex -mx-2${!animateHeight ? " h-full" : ""}`}
